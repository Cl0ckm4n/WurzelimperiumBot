...
class Http(object):
    ...
    def is_megafruit_available(self):
        try:
            response, content = self.__http.sendRequest(f'ajax/ajax.php?do=citymap_init&token={self.__http.token()}')
            self.__http.checkIfHTTPStateIsOK(response)
            content = self.__http.generateJSONContentAndCheckForOK(content)
            if 'snailracing' in content['data']['location']:
                return content['data']['location']['megafruit']['bought'] == 1
            else:
                return False
        except:
            raise

    def is_decogarden2_available(self):
        try:
            response, content = self.__http.sendRequest(f'ajax/ajax.php?do=citymap_init&token={self.__http.token()}')
            self.__http.checkIfHTTPStateIsOK(response)
            content = self.__http.generateJSONContentAndCheckForOK(content)
            if 'snailracing' in content['data']['location']:
                return content['data']['location']['decogarden2']['bought'] == 1
            else:
                return False
        except:
            raise